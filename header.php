...
<title><?php
	...
	if ( $paged >= 2 || $page >= 2 )
		echo ' | ' . __('Page', 'infocongo') . max($paged, $page);

	?></title>
<link rel="profile" href="http://gmpg.org/xfn/11" />
<link rel="stylesheet" type="text/css" media="all" href="<?php bloginfo('stylesheet_url'); ?>" />
<link rel="pingback" href="<?php bloginfo('pingback_url'); ?>" />
<link rel="shortcut icon" href="<?php bloginfo('stylesheet_directory'); ?>/img/favicon2.ico" type="image/x-icon" />
<?php wp_head(); ?>
</head>
					...
					<nav>
						<ul>
							<?php wp_nav_menu(array(
								'items_wrap' => '<ul id="%1$s" class="%2$s">%3$s<!--<li><a href="#submit" class="submit-story">' . __('Submit a story', 'infocongo') . '</a></li>--></ul>'
							)); ?>
						</ul>
